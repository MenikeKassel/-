...
def export_data_to_csv(data):
  ...
  data.to_csv("个人关联规则.csv", encoding="utf_8_sig")
if __name__ == '__main__':
    ...
    # tmp_data = data['workingexp'].str.strip()
    # data['workingexp'] = tmp_data
    # capacity = data[['company_name', 'workingexp', 'edu']].drop_duplicates(subset=['company_name'], keep='first')
    # print(capacity[10:])
    #
    # capacity['workingexp'] = tmp_data
    # print(capacity.shape[0])
    #
    # df = capacity['workingexp'].value_counts()
    ...
    # pie.render("学历.html")
    #
    # gp = data.pivot_table('s_average', index='workingexp', columns='edu', aggfunc='mean', margins=True,
    #                       fill_value=0)
    # order = ['初中及以下', '中技', '中专', '高中', '大专', '本科', '硕士', '博士', 'All']
    # gp = gp[order]
    # c = draw_heat(gp, gp.values.max())
    # c.render("学历薪资.html")
    # print("最高薪资对应的工作经验及学历如下:")
    # print(data[data.s_max == data.s_max.max()][['workingexp', 'edu']])
    c_s = data[['workingexp', 'edu', 's_average_sum']]
    c_s = encode(c_s)
